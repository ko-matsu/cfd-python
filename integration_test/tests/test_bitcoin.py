import unittest
from helper import RpcWrapper, get_utxo, long_sleep
...
def test_sequence_locktime(test_obj: 'TestBitcoin', time_test: bool):
    btc_rpc = test_obj.conn.get_rpc()
    main_addr = test_obj.addr_dic['main']
    main_pk, _ = SchnorrPubkey.from_pubkey(str(main_addr.pubkey))
    pkh_addr = test_obj.addr_dic['p2pkh']
    ...
    for txin in tx2.txin_list:
        for utxo in utxo_list:
            if utxo.outpoint == txin.outpoint:
                join_utxo_list.append(utxo)
    for index, txin in enumerate(tx2.txin_list):
        tx2.update_sequence(txin.outpoint, height_lock=3)  # set confirmation
    for index, txin in enumerate(tx2.txin_list):
        utxo = search_utxos(test_obj, join_utxo_list, txin.outpoint)
        if index == 0:
            sighash = tx2.get_sighash(
    ...
    try:
        btc_rpc.sendrawtransaction(str(tx2))
        test_obj.assertTrue(False, 'Fail: broadcast success')
    except JSONRPCException as e:
        print('success catch exception.:{}'.format(e))

    # generate block
    btc_rpc.generatetoaddress(1, fee_addr)
    time.sleep(2)
    # broadcast
    btc_rpc.sendrawtransaction(str(tx2))
    # generate block
    ...
    # generate block for time lock (12 block average time)
    btc_rpc.generatetoaddress(24, fee_addr)
    time.sleep(2)
    # send taproot singleKey2
    txin_list = []
    txin_utxo_list = []
    ...
    for txin in tx2.txin_list:
        for utxo in utxo_list:
            if utxo.outpoint == txin.outpoint:
                join_utxo_list.append(utxo)
    for index, txin in enumerate(tx2.txin_list):
        # set confirmation time (1=512 sec)
        tx2.update_sequence(txin.outpoint, time_lock=1)
    for index, txin in enumerate(tx2.txin_list):
        utxo = search_utxos(test_obj, join_utxo_list, txin.outpoint)
        if index == 0:
            sighash = tx2.get_sighash(
    ...
    if time_test:  # FIXME: Reconfirm testing methods.
        # wait
        print(f'sleep start: ${datetime.datetime.now()}')
        long_sleep(btc_rpc, 600)  # over 512 sec
        print(f'sleep end: ${datetime.datetime.now()}')
        # broadcast
        print(Transaction.parse_to_json(str(tx2), network=NETWORK))
        btc_rpc.sendrawtransaction(str(tx2))
        # generate block
    ...
    tx2 = Transaction.create(2, height+1, txin_list, txouts2)
    main_addr = test_obj.addr_dic['main']
    utxos = get_utxo(btc_rpc, [fee_addr])
    utxo_list = convert_bitcoin_utxos(test_obj, utxos)
    tx2.fund_raw_transaction(txin_utxo_list, utxo_list, fee_addr,
...
class TestBitcoin(unittest.TestCase):
    ...
    def test_bitcoin(self):
        ...
        test_sequence_locktime(self, False)
